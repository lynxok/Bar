...
export function POS() {
  ...
  const getEstimatedWaitTime = () => {
    ...
    cart.forEach(cartItem => {
      ...
      if (prod && prod.prepCount && prod.prepCount >= 20) {
  ...
  const handleProcessPayment = async () => {
    ...
    await closeOrder('POS_FAST', selectedPaymentMethod, loyaltyId);
    await LoggerService.audit('SALE', 'POS', `Venta rápida procesada: $${total.toFixed(2)} (${selectedPaymentMethod})`);
  ...
  const currentCustomer = customers.find(c => c.dni === loyaltyId || c.id === loyaltyId);

  const handleRedeem = async (prize: any) => {
    if (!currentCustomer) return;
    try {
      await redeemPoints(currentCustomer.dni, prize.pointsCost);
    ...
    <div className="h-[calc(100vh-64px)] p-6 grid grid-cols-12 gap-6 overflow-hidden">
      ...
      <section className="col-span-12 xl:col-span-8 flex flex-col gap-6 overflow-hidden max-h-full">
          ...
          <div className="flex bg-surface-container-low p-1 rounded-xl shadow-sm border border-slate-200">
            <button 
              className={cn(
                ...
                orderType === 'dine-in' ? "bg-indigo-600 text-white" : "text-slate-500 hover:bg-slate-200"
              )}
            ...
            <button 
              className={cn(
                "px-4 py-2 rounded-lg text-label-caps font-label-caps flex items-center gap-2 transition-colors",
                orderType === 'takeaway' ? "bg-indigo-600 text-white" : "text-slate-500 hover:bg-slate-200"
              )}
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide shrink-0">
          <button className="whitespace-nowrap px-6 py-2 bg-indigo-600 text-white rounded-full font-bold text-xs uppercase tracking-wider">Todo</button>
          <button className="whitespace-nowrap px-6 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors rounded-full font-bold text-xs uppercase tracking-wider">Entradas</button>
          <button className="whitespace-nowrap px-6 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors rounded-full font-bold text-xs uppercase tracking-wider">Platos Fuertes</button>
          <button className="whitespace-nowrap px-6 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors rounded-full font-bold text-xs uppercase tracking-wider">Cervezas</button>
        </div>
      ...
      <section className="col-span-12 xl:col-span-4 bg-white border border-slate-200 rounded-2xl shadow-xl flex flex-col overflow-hidden max-h-full">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {orderType === 'dine-in' ? 'Pedido Rápido' : 'Para Llevar'}
            </p>
          </div>
          <button 
            onClick={() => setCart([])}
          ...
          {cart.length === 0 ? (
            ...
            cart.map(item => (
              <div key={item.id} className="flex items-center gap-4 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <div className="w-10 h-10 rounded-lg bg-white border border-slate-200 shrink-0 flex items-center justify-center font-bold text-indigo-600">
                  {item.qty}x
                </div>
                ...
                <div className="flex items-center gap-1">
                  <button 
                    ...
                    className="w-8 h-8 rounded-lg border border-slate-300 flex items-center justify-center hover:bg-white active:scale-95 text-slate-500"
                  >
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div className="space-y-2 mb-6">
            ...
            {cart.length > 0 && (
              <div className="flex justify-between items-center bg-orange-50 p-3 rounded-xl mt-4 border border-orange-100">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <button 
              ...
              className="w-full h-14 bg-slate-900 text-white text-sm font-bold rounded-xl shadow-lg hover:bg-slate-800 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
            ...
            <button 
              ...
              disabled={cart.length === 0}
              className="w-full h-14 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
            ...
            <div className="p-10 space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fidelización de Clientes</label>
                  <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">+10% PUNTOS</span>
                </div>
                ...
                {currentCustomer && (
                  <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100 flex items-center justify-between animate-in fade-in slide-in-from-top-2 duration-300">
                    <div>
                      ...
                      <p className="text-xs font-medium text-indigo-600">{currentCustomer.points || 0} puntos disponibles</p>
                    </div>
                    <div className="flex gap-2">
                      {rewards.filter(r => r.pointsCost <= (currentCustomer.points || 0)).map(prize => (
                        <button 
                          key={prize.id}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                <Barcode className="w-5 h-5 text-indigo-600" /> Escáner
              </h3>
            ...
            <div className="p-8 flex flex-col items-center text-center">
              ...
              <h4 className="text-lg font-bold text-slate-800 mb-2">Lector de Barras</h4>
              <p className="text-sm text-slate-500 mb-8">El producto se añadirá automáticamente al pedido.</p>
              <div className="w-full flex gap-2">
                <input type="text" autoFocus className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none" placeholder="Código..." />
                <button className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-bold">Añadir</button>
              </div>
            </div>
          </div>
        </div>
